...
- [x] Verify the eight critical local source files against the archived server code snapshot. The remaining helper files still need a final server-vs-release comparison before tagging a reproducibility release.
...
- [ ] Publish the 96 uniquely named checkpoint binaries and original result JSON bundle as versioned, checksummed assets, then verify every public download and link the immutable release tag in the README. Do not commit the binaries to ordinary Git history.
