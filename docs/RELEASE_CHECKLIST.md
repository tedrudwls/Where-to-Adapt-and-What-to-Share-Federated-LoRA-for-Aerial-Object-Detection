...
- [ ] Run the narrower [qualitative test renderer](QUALITATIVE_TEST.md) on the historical server, inspect its two prediction-blind, source-disjoint test cases, verify the figure and prediction manifest, and only then add the approved image to the manuscript/README. Its local CPU-only tests do not validate CUDA/Ultralytics inference.
