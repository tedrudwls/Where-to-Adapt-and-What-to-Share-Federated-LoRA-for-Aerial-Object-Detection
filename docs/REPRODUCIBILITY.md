...
The 96 original selected checkpoints and complete result JSON bundle are not presently in this source tree. A release must link immutable artifacts with checksums before a reader can rerun *evaluation without retraining*. A full recomputation from scratch remains possible if the dataset, verified pretrained weight, hardware and software environment are available. Exact bitwise replay is not promised across different CUDA installations or GPU scheduling.
