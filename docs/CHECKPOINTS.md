...
The source tree has a **narrow, read-only qualitative rendering CLI** for five seed-42 FL models; see [Held-out test qualitative comparison](QUALITATIVE_TEST.md). It is not a general checkpoint evaluation or AP recomputation CLI, and its server-side model inference still requires validation. Do not use `main.py --resume` against archived primary directories as an evaluation shortcut: that path writes evaluation manifests, logs, plots and result JSONs. The existing data validator also binds a manifest to its original absolute data root and generated YOLO tree digests. Evaluation on a relocated checkout needs a separately validated relocation/reconstruction protocol, or a path-equivalent historical data tree. This is a release gate, not a reason to modify old manifests in place.
